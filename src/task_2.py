...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    while i < len(sorted_jobs):
        ...
        while (
            i < len(sorted_jobs)
            and current_volume + sorted_jobs[i]["volume"] <= constraints["max_volume"]
            and current_items + 1 <= constraints["max_items"]
        ):
            current_volume += sorted_jobs[i]["volume"]
            current_items += 1
            current_times.append(sorted_jobs[i]["print_time"])
            group_ids.append(sorted_jobs[i]["id"])
            i += 1

        if current_times:
            total_time += max(current_times)
            print_order.extend(group_ids)
        else:
            total_time += sorted_jobs[i]["print_time"]
            print_order.append(sorted_jobs[i]["id"])
            i += 1
